...
import duplicateErrorsReportAnalyzedExpected from './eslintReport-duplicate-errors-analyzed'
...
describe('ESLint report JSON to Analyzed report', () => {
  ...
  it('Handles duplicate error messages correctly without repeating them in markdown', async () => {
    const testReportPath = `${cwd}/src/__tests__/eslintReport-duplicate-errors.json`
    const reportJS = await eslintJsonReportToJs(testReportPath)
    const analyzedReport = getAnalyzedReport(reportJS)
    expect(analyzedReport).toEqual(duplicateErrorsReportAnalyzedExpected)
  })
})
